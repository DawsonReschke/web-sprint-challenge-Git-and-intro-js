...
//     return artist; 
//     // let born = Number(artist.years.split('-')[0])
//     // let died = Number(artist.years.split('-')[1])
//     // if(born >= 1900 && died <= 2000){
//     //   twenties.push(artist); 
//     // }
//   }
//   return twenties
// }
...
for(let i = 0; i < array.length; i++){
  let born = Number(array[i].years.split('-')[0])
  let died = Number(array[i].years.split('-')[1])
  if(born >= 1900 && died <= 2000){
...
function addArtist(arr) {
  arr.push({ 
    ...
    bio: "Commodo non irure cupidatat sunt officia anim amet excepteur exercitation in amet in deserunt veniam. Anim velit ipsum minim occaecat excepteur in enim ad adipisicing officia do. Ipsum officia eiusmod commodo mollit ea commodo."
  })
  return arr
}
...
*/
function artistInfo(arr,_name){
  let artistsBiosArray = []
  for(let i = 0; i < arr.length; i++){
    if(arr[i].name == _name){
      artistsBiosArray.push(arr[i].bio); 
    }
  }
  return artistsBiosArray; 
}
...
function artistByCountry(arr,_nationality){
  let nationsArt = []
  for(let i =0; i < arr.length; i++){
    if(arr[i].nationality == _nationality){
      nationsArt.push(arr[i].name); 
    }
  }
  return nationsArt; 
}
